...
class PrimeController extends Controller
{
    ...
    public function actionUpdatePrime(){
        if(Yii::app()->request->isAjaxRequest){
            ...
            Prime::model()->byRoleId($role_id)->deleteAll();
            if(count($resource)){
                foreach ($resource as $k => $v) {
                    ...
                    $prime->resource_id = $k;
                    if(!$prime->validate()){
                        echo json_encode(array('success' => false, 'text' => '参数验证失败'));
                        Yii::app()->end();
                    }else{
                        $prime->save();
                    }
            ...
            echo json_encode(array('success' => true, 'text' => '保存成功'));
            Yii::app()->end();
        }else{
